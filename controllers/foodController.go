...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while listening food items"})
		}
		var allFoods []bson.M
...
} // Controller Function Adjustment
func GetFoodsByMenuID() gin.HandlerFunc {
	return func(c *gin.Context) {
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		defer cancel()

		// Parse the menu ID from the request parameters
		...
		if err := cursor.Err(); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occurred while iterating through food items"})
			return
		}
