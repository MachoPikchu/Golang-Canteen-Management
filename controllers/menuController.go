...
func CreateMenu() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if insertErr != nil {
			msg := fmt.Sprintf("menu itmem was not created ")
			c.JSON(http.StatusInternalServerError, gin.H{"eror": msg})
			return
		}
		defer cancel()
...
func UpdateMenu() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if menu.Start_Date.IsZero() && menu.End_Date.IsZero() {
			if !inTimeSpan(*&menu.Start_Date, *&menu.End_Date, time.Now()) { //added *& to start date and end date
