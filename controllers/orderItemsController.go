...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error pccurress while listing order item"})
			return
		}
...
func CreateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, orderItem := range orderItemPack.Order_items {
			...
			var num = toFixed(orderItem.Unit_price, 2) //var num = toFixed(*orderItem.Unit_price, 2)
			orderItem.Unit_price = num                 //&num
